...
%An answer for linear $\fb$ that depend sparsely on $\z$ is available. Let $\rfb \equiv \rfb(\rb, n)$ denote the rate of prediction error for $\regbh$ given the approximation rate $\rb$, so that $\|\regbh - \regb\|_{\gramhat} \lesssim_\Prb \rfb$. If the family of approximating functions is taken to be 
